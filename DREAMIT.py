# python3 DREAMIT.py -json_dictionary path/to/paul_toy_data.json -tf_dict path/to/trrust_rawdata.mouse.tsv -threads 20 -outdir testtoy
########### CommandLine ############################
import pandas as pd
...
class binData:
    ...
    def __init__(self, branch, outdir, core_cut, number_of_bins, gam, bin_by_var, even_distribution, tf_dict, data_dict):
        self.branch = branch
        self.outdir = outdir
...
class GAMsmooth:
    ...
    def makeSubExp(self, fraction, rawdf, gene):
        # retrieve a subsample of 80% of the total data
        dfi = rawdf.index.tolist()
        subs = rawdf.iloc[np.random.choice(dfi, math.ceil(len(dfi) * fraction), replace=False)]
        subexp = subs[gene].tolist()
        return subexp
    ...
    def splineParamSelection(self):
        ...
        self.var_max = max(self.var)
        cellnum = len(self.var)
        if cellnum < 100:  # now 100 cells total, was 30 cells per 4 bin minimum
            print('Escape case')
        #maxbin, self.weights = self.maxbinsearch()
        smoothing_factors = [0.01, 0.05, 0.1, 0.25, 0.5, 1, 2.5, 5, 7.5, 10]
        ...
        for gene in genelist:
            ...
            sf_aic = []
            genedict[gene] = {}
            gexp = rawexp[gene].tolist()
            for s in smoothing_factors:
                genedict[gene][s] = [], [], [], []
                spl = interpolate.UnivariateSpline(range(len(gexp)), gexp, s=s)  # doesn't seem like I can use ptime because of duplicate 0's
                splineexp = spl(range(len(gexp)))
                splineexp[splineexp < 0] = 0
                knots = len(spl.get_knots())
                nLL = spl.get_residual()  # should be weighted residuals
                spl_derivative = spl.derivative()
                derv = spl_derivative(range(len(gexp)))
                rough = derv[-1] - derv[0]  # end - start y derivative values
                # make sure that ALL genes have an AIC don't want unfair smoothing
                AIC = (2 * knots) + (2 * nLL) + (2 * s * rough)
                sf_aic.append(AIC)
                ...
                # make subsample splines to test robustness
                for i in range(30):
                    subexp = self.makeSubExp(0.8, rawexp, gene)
                    subspl = interpolate.UnivariateSpline(range(len(subexp)), subexp, s=s)
                    subsplineexp = subspl(range(len(subexp)))
                    subsplineexp[subsplineexp < 0] = 0
                    subcoeffs = subspl.get_coeffs()
                    genedict[gene][s][0].append(subcoeffs.tolist())
                    subknots = subspl.get_knots()
                    genedict[gene][s][1].append(subknots.tolist())
                    subres = subspl.get_residual()
                    genedict[gene][s][2].append(subres)
                    genedict[gene][s][3].append(subsplineexp.tolist())
                d = {}
                pcoeffvar = []
                for elem in genedict[gene][s][0]:
                    for i in range(len(elem)):
                        if i in d:
                            d[i].append(elem[i])
                        else:
                ...
                for k, v in d.items():
                    if np.std(v) > 0:
                        ...
                        pcoeffvar.append(0)
                coeffl.append(np.mean(pcoeffvar))
            if len(allaic) == 0:
                allcoeff = coeffl
                allaic = sf_aic
            else:
                allcoeff = [coeffl[i]+allcoeff[i] for i in range(len(coeffl))]
                allaic = [sf_aic[i]+allaic[i] for i in range(len(sf_aic))]
        distancel = []
        #distancel2 = []
        #writeSpline(allaic, genedict, self.outdir, self.branch)
        #sys.stdout.flush()
        avgaic = [i/len(genelist) for i in allaic]
        avgcoeff = [i/len(genelist) for i in allcoeff]
        maxaic = max(avgaic) #sf_by_bin.max().max()
        maxcoeff = max(avgcoeff) #allcoeff.max().max()
        #coeff_thresh = 1
        for sfi in range(len(avgaic)):
            distancel.append(np.sqrt(((avgaic[sfi] / maxaic) ** 2) + ((avgcoeff[sfi] / maxcoeff) ** 2)))
        print('avgcoeff', self.branch, avgcoeff)
        print('d', self.branch, distancel)
        sys.stdout.flush()
        opt_param = distancel.index(min(distancel))
        print('Branch', self.branch, 'Smoothing Factor of ', smoothing_factors[opt_param])
        return smoothing_factors[opt_param]
    ...
    def smoother(self):
        ...
        #    print('stopping', self.branch)
        #    return self.smoothGeneBinDict, self.number_of_bins, self.tf_dict, self.s_param
        self.s_param = self.splineParamSelection()
        ...
        self.tf_dict['Random3'] = background3
        #### don't exclude Random!!! ####
        self.getData()  # binning -> getData

        for each_gene in list(self.data_dict[self.branch].keys())[9:]:  # should be only genes
            gene_val = list(self.data_dict[self.branch][each_gene])  # expression
            x = self.var  # ptime
            x, gene_val = zip(*sorted(zip(x, gene_val)))
            if len(x) >= 4 and len(gene_val) >= 4:
                spl = interpolate.UnivariateSpline(range(len(gene_val)), gene_val, s=self.s_param)
                gene_spline = spl(x)
                gene_spline[gene_spline < 0] = 0
                # self.smoothGeneBinDict[each_gene] = gene_spline.tolist()
                minval = min(self.data_dict[self.branch][each_gene])
                m = round(float(minval), 4)
                self.smoothGeneBinDict[each_gene] = [x if x > m else m for x in gene_spline.tolist()]
        randlist = ['Random1', 'Random2', 'Random3']
        for i in range(len(randlist)):
            gene_val = list(self.data_dict[self.branch][self.randfactors[i]])  # expression
            x = self.var  # ptime
            x, gene_val = zip(*sorted(zip(x, gene_val)))
            if len(x) >= 4 and len(gene_val) >= 4:
                spl = interpolate.UnivariateSpline(range(len(gene_val)), gene_val, s=self.s_param)
                ...
                gene_spline[gene_spline < 0] = 0
                # self.smoothGeneBinDict[each_gene] = gene_spline.tolist()
                minval = min(self.data_dict[self.branch][self.randfactors[i]])
                m = round(float(minval), 4)
                self.smoothGeneBinDict[randlist[i]] = [x if x > m else m for x in gene_spline.tolist()]
        #self.smoothGeneBinDict['psuedotime_mean'] = ptime_med
        #self.smoothGeneBinDict['psuedotime_med'] = ptime_med
        writeData(self.smoothGeneBinDict, self.branch, 'Smoothed_Expression', '', self.outdir, self.number_of_bins,
                  self.s_param, len(self.var))
...
class correlationCalculations:
            ...
            if np.count_nonzero(factarr[:step]) != 0 and np.count_nonzero(
                ...
                if self.gam == 'bins':
                    thismax = self.calc_MI(factarr[:step], tararr[len(self.smoothGeneBinDict[tar_name]) - step:],
                                       self.number_of_bins)
                if self.gam == 'nobins':
                    thismax = self.calc_MI(factarr[:step], tararr[len(self.smoothGeneBinDict[tar_name]) - step:], 20)
    ...
    def roller(self):
        ...
        if self.gam == 'nobins':
            self.smoothGeneBinDict, self.number_of_bins, self.tf_dict, self.s_param = GAMsmooth(self.branch, self.outdir,
                                                                                              self.core_cut,
                                                                                              self.number_of_bins,
                                                                                              self.gam,
                                                                                              self.bin_by_var,
            ...
            if factor in self.smoothGeneBinDict:
                ...
                for target_i in targets:
                    if target_i[0] in self.smoothGeneBinDict:
                        ...
                        if self.gam == 'nobins':
                            self.mutualinfo[factor][target_i[0]] = self.calc_MI(f_heat2, tar_heat2, 20)  # updated MI kbased on quantization with 20 bins
                        if self.gam == 'bins':
...
class ksStat:
    ...
    def loadDictionaries(self):
        ...
        self.factor2target_dtw, self.regulation, self.Pprune, self.Sprune, self.RollPprune, self.RollSprune, \
        self.DTWprune, self.MIprune, self.RollDTWprune, self.RollMIprune, self.number_of_bins, self.s_param = \
            correlationCalculations(self.branch, self.outdir, self.core_cut, self.number_of_bins, self.gam,
                                    self.bin_by_var, self.even_distribution, self.tf_dict, self.data_dict).roller()
        return 'go'
        #except TypeError:
        #    print('stop')
        #    return 'stop'
                        ...
                        for tar in targets:
                            if tar[0] in self.prune_vals[factor]:
                                if method_type == "Pearson_Correlation" or method_type == "Spearman_Correlation" or method_type == "Mutual_Information":
                                    perc25 = np.percentile(self.prune_distribution[factor], self.core_cut)
                                ...
                                if method_type == "Dynamic_Time_Warping":
                                    perc75 = np.percentile(self.prune_distribution[factor], 100-self.core_cut)
...
def main(myCommandLine=None):
    ...
    try:
        ...
        result = pool.starmap(ksStat, zip(branches, repeat(myCommandLine.args.outdir),
                                          repeat(myCommandLine.args.core_cut),
                                          repeat(myCommandLine.args.number_of_bins),
                                          repeat(myCommandLine.args.gam),
